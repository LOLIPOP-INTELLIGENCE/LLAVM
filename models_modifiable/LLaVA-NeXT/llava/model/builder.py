...
# Setup code for the llava model to load the correct model architecture, set up multimodal components, configuring tokenizers, manage model quantization
...
from llava.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, DEFAULT_AUDIO_START_TOKEN, DEFAULT_AUDIO_END_TOKEN, DEFAULT_AUDIO_PATCH_TOKEN
from llava.utils import rank0_print

# init parameters: paths to model, quantisation options (4 and 8 bit), etc
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    # setup model loading params for quantisation preference
    if load_8bit:
        kwargs["load_in_8bit"] = True
    elif load_4bit:
        kwargs["load_in_4bit"] = True
        # BitsAndBytes used as a quantisation technique
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            # each model (mistral, mixtral, gemma, llama) has its own loading config
        ...
        # no lora present in model config
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        # special tokens to identify image tokens in the model's inputs
        if mm_use_im_patch_token:
            tokenizer.add_tokens([DEFAULT_IMAGE_PATCH_TOKEN], special_tokens=True)
        if mm_use_im_start_end:
            tokenizer.add_tokens([DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN], special_tokens=True)

        # mm_use_audio_patch = True
        # mm_use_audio_start_end = True
        # if mm_use_audio_patch:
        ...
        # load vision tower for processing vision capabilities
    ...
    # load context length for model else set it to 2048 tokens
